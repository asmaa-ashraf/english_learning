...
dictionary1=ps.DataFrame({'word':[],'definition':[],'desc':[],'no':[]})
...
class Dictionary:
	def __init__(self):
		self.dictionary=ps.DataFrame({'word':[],'definition':[],'desc':[],'no':[]})
	...
	def search(self,word):
		return self.dictionary.loc(word)
	def write(self):
		self.dictionary.to_csv('dictionary.csv')
	def read(self,file):
		self.dictionary=ps.read_csv('dictionary.csv')
	def get(self):
